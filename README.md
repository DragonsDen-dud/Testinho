...
- **AI reports** — Claude-powered reflection, entirely bring-your-own-key (this is a local-first, no-account app, so there's no shared server-side key to hold): a `freeform_query` you can ask about any specific habit from its edit sheet, and a `scheduled_template` weekly report that checks on every app open and generates itself once past a configurable day-of-week, surfaced on Dashboard without a button press. Both are fed the already-computed Article 26/35 `autoStats` as given facts rather than asking the model to re-derive them from raw logs. An optional per-Space "North Star" statement can be attached to any single call, but only when explicitly checked for that call — never a sticky default (Articles 4, 12, 16, 38)

Deferred to later passes per the contract's build plan: photo attachments (Article 23), Calendar `.ics` export, Telegram reminders, voice input, cross-Space analytics, month-scope Scheduled AI Reports (see note below), and the remaining polish items in Section H.
...
- **Month-scope Scheduled AI Reports are not implemented.** Article 12 describes both week and month cadences, but only a week-scope `autoStats` aggregator (`upsertWeeklyAutoStats`) exists from the previous pass — building a parallel monthly one wasn't asked for this round, so the scheduled-report trigger only ever fires the weekly report. Flagging this rather than quietly building an unrequested monthly pipeline or silently pretending Article 12 is fully covered.
- **The Connectivity Check module (E.9) did not already exist.** It was referenced as already-shipped groundwork in this round's assignment, but no such module — stub or otherwise — was present anywhere in the codebase before this pass. It's now built into the Settings → AI reports section (a small connected/unreachable indicator backed by `checkAiHealth()` against the proxy's own liveness endpoint, not a real Anthropic call).
- **North Star is a new, minimal addition**, not something that existed before this round. Article 16 assumes North Star content can be optionally attached, but nothing in the data model held such text — so `Space.northStar` (an optional free-text field, editable from Space edit) was added specifically so the opt-in toggle has real content to attach instead of being a no-op.
